...
export default function Activity5Results() {
  ...
  const normalizedResults = parsedResults.map((item: any, index: number) => ({
    ...
    movementIntensity: item.movementIntensity ?? 0,
    averageAngularVelocity: item.averageAngularVelocity ?? 0,
    speed: item.speed ?? 0,
    smoothness: item.smoothness ?? 0,
    rangeOfMotion: item.rangeOfMotion ?? 0,
  ...
  const fastestSpeed =
    Number(params.fastestSpeed);

  const highestSmoothness =
    Number(params.highestSmoothness);

  const largestRangeOfMotion =
    Number(params.largestRangeOfMotion);

  const bestPerformanceScore =
    Number(params.bestPerformanceScore);
    ...
    const csvContent = [
      ...
      ...normalizedResults.map((item: any) =>
        [
          item.iterationNo,
          item.movementType,
          item.movementIntensity,
          item.averageAngularVelocity,
          item.speed,
          item.smoothness,
          item.rangeOfMotion,
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => 
            router.push({
              pathname: '/activities/activity5/feedback',
              params: {
