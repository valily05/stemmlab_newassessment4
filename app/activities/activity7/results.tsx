...
export default function Activity7Results() {
  ...
  const normalizedResults = parsedResults.map((item: any, index: number) => ({
    ...
    exerciseType: item.exerciseType ?? 'N/A',
    restBreathingRate: item.restBreathingRate ?? 0,
		postBreathingRate: item.postBreathingRate ?? 0,
    ...item,
  }));
  ...
  const averageBreathingRate =
    Number(params.averageBreathingRate);

  const peakBreathingRate =
    Number(params.peakBreathingRate);
    ...
    const csvContent = [
      ...
      ...normalizedResults.map((item: any) =>
        [
          item.iterationNo,
          item.exerciseType,
          item.restBreathingRate,
          item.postBreathingRate
        ].join(',')
      ),
    ].join('\n');
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => 
            router.push({
              pathname: '/activities/activity7/feedback',
              params: {
